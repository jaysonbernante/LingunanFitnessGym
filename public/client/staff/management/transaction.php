...
<style>
    ...
    /* Responsive top-products table: stacked cards on mobile */
    .tp-table-container { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }
    @media (max-width: 600px) {
        .tp-table { min-width: 0; }
        .tp-table thead { display: none; }
        .tp-table tbody tr { display: block; margin-bottom: 12px; border: 1px solid rgba(255,255,255,0.03); border-radius: 8px; padding: 8px; background: #1b1b1b; }
        .tp-table tbody td { display: flex; justify-content: space-between; padding: 8px 10px; border-bottom: none; }
        .tp-table tbody td::before { content: attr(data-label); color: #f5c518; font-weight: 700; margin-right: 8px; width: 120px; flex: 0 0 120px; }
    }
</style>
<body>
...
function renderTopTable(rows) {
    ...
    rows.forEach((r, i) => {
        ...
        html += `<tr>
            <td class="tp-rank" data-label="#">${i + 1}</td>
            <td data-label="Product">${r.product_name}</td>
            <td data-label="Qty Sold" style="text-align:center;">${r.total_qty}</td>
            <td data-label="Revenue" style="color:#f5c518;font-weight:700;">${fmt(r.total_revenue)}</td>
            <td data-label="Share"><div class="tp-bar-wrap"><div class="tp-bar" style="width:${pct}%"></div></div></td>
        </tr>`;
    });
